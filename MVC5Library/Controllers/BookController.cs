...
namespace MVC5Library.Controllers
{
    public class BookController : Controller
    {
        ...
        {
            List<SelectListItem> Category = (from i in Context.TBLCategory.Where(x => x.CategoryStatus == true).ToList()
            ...
            List<SelectListItem> Author = (from i in Context.TBLAuthor.Where(x => x.AuthorStatus == true).ToList()
                                           select new SelectListItem
                                           {
        ...
        [HttpPost]
        public ActionResult BookAdd(TBLBook tBLBook, HttpPostedFileBase BookPhoto)
        {
            string BookPhotoPath = Path.Combine(Server.MapPath("~/VitrinTema/assets/img/portfolio/"), Path.GetFileName(BookPhoto.FileName));
            BookPhoto.SaveAs(BookPhotoPath);
            ...
            tBLBook.BookPhoto = "/VitrinTema/assets/img/portfolio/" + BookPhoto.FileName;
